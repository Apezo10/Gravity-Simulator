...
class Planet {
    ...
    // Magnitude of the current velocity vector, in metres per second.
    double getSpeed() const { return hypot(xVel, yVel); }
...
int main() {
    Planet body(0, 0, 3000, -4000, 1, 1);
    if (body.getSpeed() != 5000) return 1;
    body.setAccel(1, 0);
    body.update();
    if (abs(body.getSpeed() - hypot(6600.0, -4000.0)) > 1e-8) return 2;
    cout << "Speed magnitude and live velocity updates passed.\n";
}

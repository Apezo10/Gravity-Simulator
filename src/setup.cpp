...
bool getInfo(Planet& body) {
    ...
    double xPos{}, yPos{}, xVel{}, yVel{}, radius{}, mass{};
...
void printInfo(const Planet& body) {
    ...
    cout << "X vel: " << body.getXvel() << " (m/s)\n";
    cout << "Y vel: " << body.getYvel() << " (m/s)\n";
    cout << "Radius: " << body.getRad() << "(m)\n";
}
bool getPlanets(PlanetSystem& system) {
    ...
    for (int i=0; i<size; i++) {
        cout << "\nPlanet " << i+1 << ":\n";
        if (!getInfo(candidates[i])) return false;
    }
...
void printPlanets(const PlanetSystem& system) {
    const auto& planets = system.getParSystem();
